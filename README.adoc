...
# Yet Another Picoprobe Clone for RP2040 Zero

## Overview

There is a chain of forks and clones which lead to this project. This time it was forked from 
https://github.com/rgrr/yapicoprobe[YAPicoprobe]. And I want to say big thanks to it's author.

I don't plan to develop the code further, but want to write down what was done to get this 
probe working. Basically I wanted something like a logic analyzer and analog tracker, 
plus DAPLink to get it possible to program different devices based on ARM MCU.

I switched off all the debug code - tty console, debug output. I switched off the 
functionality related to segger SystemView - may be it is more stable, and can cover 
many cases, but I like the Sigrok Pulseview and want to use it instead. 

One more reason was to use cheap small board called https://www.waveshare.com/wiki/RP2040-Zero[RP2040 Zero].
It is much smaller, than original PI, buit still has enough pins for this application. And It has USB-C 
on board. 

[.text-center]
image::RP2040-Zero.png[RP2040-Zero, 60%]

Ah, yea, I removed a makefile from YAPicoprobe. This fork took configuration in the CMakeLists.txt file and 
uses CMake only for build.
...
I've made this tool in iron. I'm not ready to offer a PCB now, but teh schematics itself is there:

link:doc/extern/yapicoprobe-zero.pdf[Schematics]

I've used LM324 OPAMP to make analogue inputs protected and have a possibility to shift zero level 
for measuring 0-based signals. Additionally I've used two transistors to make TXD and RXD tolerate 
to 3.3 and 5 volts. All of it not really precise, but works and LM324 can be mounted on socket to 
change it in case of some burn.
...
As the original RP2040-Zero board has a WS2812 RGB LED on board. And this LED managemnt is quite 
sensitive to the timings, I don;t want to add more trouble to teh MCU as it was. And decide to use 
simple additional LED as it was in YAPicoprobe.
...
[source,bash]
----
git clone https://github.com/n0imaginati0n/yapicoprobe-zero.git
cd yapicoprobe-zero
...
[source,bash]
----
# it is nice idea to keep all teh generated files in sepoarate dir. it will not pollute all the project
# crewate build dir
mkdir _build
...
# ask CMake to configure and generate buil scnarios for Ninja. I like Ninja, but one can use many 
# other generators, like GNU Makefiles or MS VS. read CMake help
cmake -G Ninja ..

# ask CMake to build a project for you
cmake --build .

# got to the initial dir
cd ..
----

.Build sequence for a specific board [pico|pico_w|pico_debug_probe|waveshare_rp2040_zero]

all the same as before, but either you will specify CMake variable in the commandline at
[source,bash]
----
cmake -G Ninja .. -DPICO_BOARD=pico_w -D<option>=<value> 
----
or modify teh CmakeLists.txt file
